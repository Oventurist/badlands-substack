---
title: "GAVI"
created: 2026-08-06
updated: 2026-08-06
type: entity
tags: [vaccines, global-health, philanthropy, public-private-partnership]
sources: [raw/the-ivy-web.md, raw/the-whos-last-stand.md]
confidence: low
---

# GAVI

GAVI is a global vaccine alliance named in Badlands Media's *The Ivy Web* among the funders whose contracts allegedly compromised the neutrality of Harvard-affiliated public health researchers.[1]

## Role in the article

The article states that researchers framing the recommendations delivered to the [[world-health-organization|World Health Organization]] through the [[harvard-global-health-institute|Harvard Global Health Institute]] and the [[harvard-th-chan-school-of-public-health|T.H. Chan School of Public Health]] "were not neutral scientists," with many under contract to [[gates-foundation|Gates Foundation]] initiatives, GAVI, or consulting arms such as the McKinsey Global Institute.[1] The recommendations in question included behavioral nudging frameworks to increase vaccine uptake, justification models for quarantine extension, and algorithmic impact assessments for misinformation response.[1]

## References
1. Badlands Brief — "The Ivy Web", URL: https://badlands.substack.com/p/the-ivy-web

## Role in WHO funding critiques

Badlands Media's "The WHO's Last Stand" counted GAVI the Vaccine Alliance among the organizations whose contributions must be aggregated in order to identify [[bill-gates|Bill Gates]] as the largest funder of the [[world-health-organization|World Health Organization]], alongside the [[gates-foundation|Gates Foundation]].[2] The article used this aggregation to argue that private interests wield immense power over the WHO because most of its funding is earmarked for specific programs.[2]

2. Badlands Brief — "The WHO's Last Stand", URL: https://badlands.substack.com/p/the-whos-last-stand
